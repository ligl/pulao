...
- 核心领域模型：`bar / swing / trend / keyzone / sd / decision`
...
- 策略入口：`StruxisStrategy`（兼容别名：`PulaoStrategy`）
...
- Decision 联合使用 KeyZone 行为 + SD 阶段/质量动态调节开仓阈值
...
- `pulao/bar/*` -> `struxis/src/market/bar.rs` + `struxis/src/analysis/mtc.rs`
- `pulao/swing/*` -> `struxis/src/analysis/swing.rs`
- `pulao/trend/*` -> `struxis/src/analysis/trend.rs`
- `pulao/keyzone/*` -> `struxis/src/analysis/keyzone.rs` + `keyzone_builder.rs` + `keyzone_factory.rs`
- `pulao/sd/sd.py` -> `struxis/src/analysis/sd.rs`
- `pulao/decision/decision.py` -> `struxis/src/execution/decision.rs`
- `pulao/indicator/*` -> `struxis/src/market/indicator.rs`
- `pulao/symbol/*` -> `struxis/src/market/symbol.rs`
- `pulao/utils.py` -> `struxis/src/utils.rs`
- `pulao/logging.py` -> `struxis/src/logging.rs`
- `pulao/strategy.py` -> `struxis/src/execution/strategy.rs`
...
```rust
let mut strategy = StruxisStrategy::new("I2601");
...
- `src/{constant,events,logging,utils}.rs`：基础设施层（常量、事件、日志、通用工具）。
- `src/market/*`：市场数据层（tick/bar/receiver/symbol/indicator）。
- `src/analysis/*`：结构与评估层（mtc/swing/trend/keyzone/sd）。
- `src/execution/*`：执行层（decision/strategy）。

依赖方向（必须遵守）：

- `core`：不依赖其他业务层。
- `market`：可依赖 `core`；不可依赖 `analysis` / `execution`。
- `analysis`：可依赖 `core` + `market`；不可依赖 `execution`。
- `execution`：可依赖 `core` + `market` + `analysis`。

工程规则：

- 新增模块时，先归属层级，再决定放置目录。
- 禁止跨层反向依赖（例如 `market -> analysis`、`analysis -> execution`）。
