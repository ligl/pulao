...
for idx, row in enumerate(df.iter_rows()):
    ...
    sbar = SBar(id= len(sbar_list), symbol=bar.symbol, exchange=bar.exchange.value, timeframe=bar.interval.value, datetime=datetime, turnover=money, open_price=open, close_price=close, high_price=high, low_price=low, volume=volume)
